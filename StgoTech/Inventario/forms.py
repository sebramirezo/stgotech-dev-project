...
class ConsumosForm(ModelForm):
    ...
    consumidor_fk = forms.ModelChoiceField(queryset=Consumidor.objects.all(), widget=forms.Select(attrs={"class": "form-control","placeholder": "Ingresa Consumidor"}), label='Consumidor')
...
class DetalleForm(ModelForm):
    rcv_n = forms.CharField(widget=forms.TextInput(attrs={"class":"form-control", "placeholder": "Ingresa RCV N°"}),label='RCV N°', required=False)
    ...
    item1 = forms.CharField(widget=forms.Select(choices=ITEMS, attrs={"class": "form-control"}), label='1) Producto conforme a lo indicado en la lista de Embarque (Packing List)', required=False)
    item2 = forms.CharField(widget=forms.Select(choices=ITEMS, attrs={"class": "form-control"}), label='2) Factura del Proveedor conforme a la orden de compra o solicitud de trabajo (Invoice)', required=False)
    item3 = forms.CharField(widget=forms.Select(choices=ITEMS, attrs={"class": "form-control"}),label='3) Cartilla/Orden de Trabajo, Cartilla de prueba, Si corresponde, del taller que repara', required=False)
    item4 = forms.CharField(widget=forms.Select(choices=ITEMS, attrs={"class": "form-control"}),label='4) Producto sin daños visibles (Inspeccion Visual)', required=False)
    item5 = forms.CharField(widget=forms.Select(choices=ITEMS, attrs={"class": "form-control"}),label='5) Producto protegido en embalaje apropiado', required=False)
    item6 = forms.CharField(widget=forms.Select(choices=ITEMS, attrs={"class": "form-control"}),label='6) Placa de identificacion del componente', required=False)
    item7 = forms.CharField(widget=forms.Select(choices=ITEMS, attrs={"class": "form-control"}),label='7) Documentacion Técnica completa requerida por reglamentacion (Trazabilidad)', required=False)
    item8 = forms.CharField(widget=forms.Select(choices=ITEMS, attrs={"class": "form-control"}),label='8) Formulario FAA 8130-3', required=False)
    item9 = forms.CharField(widget=forms.Select(choices=ITEMS, attrs={"class": "form-control"}),label='9) Formulario EASA Form One o JAA Form One o CAA Form One', required=False)
    item10 = forms.CharField(widget=forms.Select(choices=ITEMS, attrs={"class": "form-control"}),label='10) Formulario DGAC Chile 8130-3', required=False)
    item11 = forms.CharField(widget=forms.Select(choices=ITEMS, attrs={"class": "form-control"}),label='11) Formulario ANAC Argentina 8130-3', required=False)
    item12 = forms.CharField(widget=forms.Select(choices=ITEMS, attrs={"class": "form-control"}),label='12) Placa o Etiqueta para Herramientas o equipos con calibracion', required=False)
    item13 = forms.CharField(widget=forms.Select(choices=ITEMS, attrs={"class": "form-control"}),label='13) Certificado de Calibración en laboratorio reconocido por el estado local', required=False)
    ...
    item14 = forms.CharField(widget=forms.Select(choices=ITEMS, attrs={"class": "form-control"}),label='14) Materiales con vida limite (Verificacion de Shelf life data y MSDS)', required=False)
    item15 = forms.CharField(widget=forms.Select(choices=ITEMS, attrs={"class": "form-control"}),label='15) Certificado de flamabilidad, si corresponde', required=False)
    ...
    item16 = forms.CharField(widget=forms.Select(choices=ITEMS, attrs={"class": "form-control"}),label='16) Certificado de conformidad  y/o Analisis', required=False)
    ...
    item22 = forms.CharField(widget=forms.Select(choices=ITEMS, attrs={"class": "form-control"}),label='22) Cartilla Mantencion CMA Autorizado', required=False)
